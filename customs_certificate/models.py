...
class CustomsCertificate(models.Model):
    person_type = models.CharField('Shaxs turi', choices=PERSON_CHOICES, max_length=3, default="J")
    seriya = models.CharField('Bojxona guvohnomasi seriyasi va raqami', max_length=10, null=True)
    ...
    class Meta:
        verbose_name = "Bojxona guvohnomasiga asosan avtotransport vositasiga raqam olish"
        verbose_name_plural = "Bojxona guvohnomasiga asosan avtotransport vositasiga raqam olish"

    def __str__(self):
        if self.seriya:
            return f'Bojxona guvohnomasi: {self.seriya}'
        else:
            return f"ERROR CUSTOMS CERTIFICATE IS NULL"