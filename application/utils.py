...
def application_right_filters(qs, request_get):
    if request_get.get('service'):
        if request_get.get('service') == 'account_statement':
            qs = qs.filter(service__title='account_statement')
            return qs
        if request_get.get('service') == 'gift_agreement':
            qs = qs.filter(service__title='gift_agreement')
            return qs
        if request_get.get('service') == 'contract_of_sale':
            qs = qs.filter(service__title='contract_of_sale')
            return qs
        if request_get.get('service') == 'replace_tp':
            qs = qs.filter(service__title='replace_tp')
            return qs
        if request_get.get('service') == 'replace_number_and_tp':
