            ...
            if request.is_ajax():
                if request.method == 'POST':
                    ...
                    if request.POST.get('process') == 'confirm':
                        ...
                        text = f"E-RIB.UZ: {application.id}-raqamli arizangiz muvvaffaqiyatli tasdiqlandi! {application.section.title} ga kelib qayd etish guvohnomasini olib ketishingizni so'raymiz. {', '.join([d.title for d in document_polices])} Qo'shimcha ma'lumot uchun tel:972800809"
                        ...
                        text = f"E-RIB.UZ: {application.id}-raqamli arizangiz {application.section.title} tomonidan ko'rib chiqish uchun qabul qilindi! Quydagilarni {application.section.title} ga keltirib topshirishingizni so'raymiz!  Qo'shimcha ma'lumot uchun tel:972800809"
